...
int modify_config()
{
  ...
  oConfigurationData.OptionalFeatureSupport = CONFIGURATION_OPTIONAL_FEATURE_DISABLECANCELSESSIONUNDERRUN | CONFIGURATION_OPTIONAL_FEATURE_ENABLENOTIFICATIONMESSAGE_INCHALL;
  ...
  if (oConfigurationData.OptionalFeatureSupport != (CONFIGURATION_OPTIONAL_FEATURE_DISABLECANCELSESSIONUNDERRUN | CONFIGURATION_OPTIONAL_FEATURE_ENABLENOTIFICATIONMESSAGE_INCHALL))
    {
...
int FT601_Read(FT_HANDLE ftHandle, void *buf, size_t len)
{
  ...
  ftStatus = FT_ReadPipe(ftHandle, 0x82, buf, len, &ulBytesRead, NULL);
  my_printf("Status rd %d, len: %d\n", ftStatus, ulBytesRead);
  return ulBytesRead;
}
...
int FT601_Write(FT_HANDLE ftHandle, void *buf, size_t len)
{
  ...
  ftStatus = FT_WritePipe(ftHandle, 0x02, buf, len, &ulBytesWritten, NULL);
  my_printf("Status wr %d, len: %d\n", ftStatus, ulBytesWritten);
